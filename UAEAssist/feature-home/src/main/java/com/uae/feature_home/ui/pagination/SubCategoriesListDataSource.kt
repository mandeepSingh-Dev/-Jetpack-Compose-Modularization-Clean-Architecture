...
class SubCategoriesListDataSource(
    private val search : String? = null,
    private val active : Int? = null,
    private val categoryId : String,
    private val homeRepository : HomeRepository,
) : PagingSource<Int, SubCategoriesListResponse.SubCategoryData>() {

    override fun getRefreshKey(state: PagingState<Int, SubCategoriesListResponse.SubCategoryData>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int,SubCategoriesListResponse.SubCategoryData> {
        val nextPage = params.key ?: 1

        val result = homeRepository.getSubCategoryListing(
            page = nextPage, limit = params.loadSize,
            search = search, active = active,categoryId = categoryId)
            .first()
